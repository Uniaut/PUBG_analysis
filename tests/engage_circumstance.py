...
def Winner_Engage_Position(match, input_kill_position): # Engage_Victim을 구할 때 1등이 죽인 Victim들만 고려
    ...
    for engage_event in engage_log: #log_player_take_damage에서 가져온 로그
        try:
            ...
            continue    
        if engage_event.victim.name in victim_name and attacker_name == chicken_player:#engage_position의 key = 공격 받은 victim.name과 같고 공격자가 1등플레이어인 상황
...
def winner_victim_engage_circumstance(engage_dictionary, input_kill_position): #기존에 있던 winner_kill_position, engage_circle까지 한번에 들어있음.
    ...
    for i in range(number_of_victims):
        ...
        for engage_event in engage_dictionary[list_victims[i]]: # engage_event = 각각의 victim에 대한 log_player_take_damage
            ...
            plt.scatter(victim_x, victim_y, color=[1,(1/number_of_victims)*i,0.4]) # Victim은 각각 색이 다름
            plt.arrow(winner_x, winner_y, victim_x-winner_x, victim_y-winner_y,color='white') # victim이 winner한테서 데미지 받았을 때 표시-> 오로지 winner가 공격자일때의 입장만 나옴->winner가 데미지 받은 상황은 안나옴
            engage_distance_max = math.dist([winner_x,winner_y], [victim_x,victim_y]) / 2 +2 # 반지름 값
            ...
            if center[0] == 0 and center[1] == 0 : #처음 교전시 두 플로이어 간의 교전 중심은 서로의 좌표/2
                ...
                y = center[1] + (winner_y + victim_y) / 2 # -> 교전 중심은 이전까지의 서로의 중간좌표 + 새로운 중간좌표 
            number_of_engagement = number_of_engagement + 1 #교전 횟수
            center=(x,y)
        killer_position_x, killer_position_y = input_kill_position[i]['x'], input_kill_position[i]['y']
        victim_killed_position_x, victim_killed_position_y = input_kill_position[i]['Victim_Location_X'], input_kill_position[i]['Victim_Location_Y']
        kill_dist = math.dist([killer_position_x, killer_position_y], [victim_killed_position_x, victim_killed_position_y])/2+0.1
