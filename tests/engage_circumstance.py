...
def Winner_Engage_Position(
    ...
    engage_position = (
        {}
    )  # 각각의 victim_name은 key, value=해당 victim의 log_player_take_damage
...
def winner_victim_engage_circumstance(
    ...
    for i in range(number_of_victims):
        ...
        for engage_event in engage_dictionary[
            ...
            ):  # 교전 중 서로간의 거리가 가장 멀었을 때로 반지름 설정할 것
                engage_circle_radius = engage_distance_max
...
if __name__ == "__main__":
    ...
    winner_victim_engage_circumstance(
        winner_engage_position, Winner_Kill_Position(match)
    )
